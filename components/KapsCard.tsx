...
const KapsCard = ({ isOfficial, isSponsored, name, category, author, imageUrl, className, ...props }: props) => {
  return (
    <div className={`relative min-w-44 w-44 h-64 rounded-lg ${className}`} {...props}>
      <div className='absolute w-full h-full bg-black rounded-lg'>
        <Image src={imageUrl || '/default-image.png'} layout='fill' objectFit='cover' alt='' className='w-36 h-64 object-cover opacity-70 rounded-lg' />
      </div>
